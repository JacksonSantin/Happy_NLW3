...
For information about options that've changed, there's always [the changelog](https://github.com/tomas/needle/releases).
 ...
 - `proxy`       : Forwards request through HTTP(s) proxy. Eg. `proxy: 'http://user:[removido]:3128'`. For more advanced proxying/tunneling use a custom `agent`, as described below.
...
```js
needle.get('https://username:[removido]', function(err, resp) {
